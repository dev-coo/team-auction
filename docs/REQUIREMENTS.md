...
- 입력:
  ...
  - 팀 수 (예: 8팀)
  - 팀당 인원 (예: 5명)
  - 팀장 정보: 이름, 포지션, 한줄소개, **팀장 포인트 (기본값: 0)**
  - 팀원 정보: 이름, 포지션, 한줄소개
- 고정값:
  - 경매 시작 시간: 15초
  - 입찰 시 추가 시간: +2초
...
- 설명: 경매 제한 시간 관리
- 입력: 없음 (자동)
- 출력: 실시간 타이머 표시 (0.1초 단위)
- 비즈니스 규칙:
  - 초기값: 15초 (150 단위, 0.1초 기준)
  - 입찰 발생 시: 현재 남은 시간 + 2초 (최대 15초)
  - 0초 도달 시: 낙찰 처리
- 예외 처리: 입찰자가 없을 경우 유찰 → 2라운드 진행 → 랜덤 배분
...
- [x] 실시간 채팅 - 전체 (FR-CHAT-001) *(미구현)*
- [x] 경매 결과 페이지 (FR-RESULT-001)

### Should Have (권장) - v1.0
- [x] 팀장 소개 페이즈 (FR-AUC-001) ✅ 완료
- [x] 경매 현황 대시보드 (FR-AUC-009~010) ✅ 완료
- [x] 유찰 처리 - 2라운드 + 랜덤 배분 ✅ 완료
...
| v1.1 | 2025-12-03 | Claude | FR-ROOM-002: 팀장 포인트 입력 기능 추가 |
| v1.2 | 2025-12-07 | Claude | 구현 완료 상태 반영, 유찰 처리 로직 추가 |
