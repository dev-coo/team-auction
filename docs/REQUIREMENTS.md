...
- 설명: 역할별 고유 초대 링크 생성
- 입력: 역할 유형 (팀장/팀원/옵저버)
- 출력: 고유 URL (예: `/join/abc123?role=captain`)
- 비즈니스 규칙:
  - 주최자 링크: 1개
  - 팀장 링크: 팀 수만큼 생성 가능
  - 팀원 링크: 1개 (다수 사용 가능)
...
| ID | 기능명 | 설명 | 우선순위 | 상태 |
|----|--------|------|----------|------|
| FR-JOIN-001 | 링크 입장 | 초대 링크로 경매방 입장 | 높음 | 예정 |
| FR-JOIN-002 | 팀장 정보 입력 | 닉네임, 포지션(필수), 한줄소개/다짐 입력 | 높음 | 예정 |
| FR-JOIN-003 | 팀원 정보 입력 | 닉네임, 포지션(필수), 능력/한줄평 입력 | 높음 | 예정 |
| FR-JOIN-004 | 옵저버 입장 | 닉네임만 입력하고 입장 | 낮음 | 예정 |
| FR-JOIN-005 | 대기실 | 경매 시작 전 대기 화면 | 중간 | 예정 |

#### 상세 설명

**FR-JOIN-001: 링크 입장**
- 설명: 초대 링크를 통해 경매방에 입장
- 입력: 초대 링크 URL
- 출력: 역할에 맞는 정보 입력 폼 표시
- 비즈니스 규칙: 유효한 링크만 입장 가능, 만료된 링크 거부
- 예외 처리: 잘못된 링크 시 에러 페이지

**FR-JOIN-002: 팀장 정보 입력**
- 설명: 팀장이 본인 프로필 등록
- 입력:
  - 닉네임 (필수)
  - 포지션 (필수): 텍스트필드로 입력 할 수 있게끔
  - 한줄소개/다짐 (선택)
- 출력: 프로필 저장 완료, 대기실 이동
- 비즈니스 규칙: 닉네임 중복 불가 (같은 경매방 내)

**FR-JOIN-003: 팀원 정보 입력**
- 설명: 팀원이 본인 프로필 등록
- 입력:
  - 닉네임 (필수)
  - 포지션 (필수): 텍스트필드로 입력 할 수 있게끔
...
- 비즈니스 규칙: 모든 참가자(주최자, 팀장, 팀원, 옵저버)가 동일한 페이즈 상태 확인 가능
...
| ID | 기능명 | 설명 | 우선순위 | 상태 |
|----|--------|------|----------|------|
| FR-CHAT-001 | 전체 채팅 | 모든 참가자가 참여하는 채팅 | 높음 | 예정 |
| FR-CHAT-002 | 팀 채팅 | 같은 팀끼리만 보이는 채팅 | 높음 | 예정 |
| FR-CHAT-003 | 채팅방 자동 이동 | 낙찰 시 팀원이 팀 채팅방으로 자동 이동 | 높음 | 예정 |
| FR-CHAT-004 | 채팅 알림 | 새 메시지 알림 | 낮음 | 예정 |

#### 상세 설명

**FR-CHAT-003: 채팅방 자동 이동**
- 설명: 팀원이 낙찰되면 해당 팀 채팅방으로 자동 이동
- 입력: 낙찰 이벤트
- 출력: 팀 채팅방 활성화, 팀장+기존 팀원과 채팅 가능
- 비즈니스 규칙:
  - 전체 채팅은 계속 참여 가능
  - 팀 채팅 탭 추가됨
